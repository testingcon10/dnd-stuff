---
tags: [race, reference]
ability_bonus: "CON +2, STR +1"
size: "Medium"
speed: 25
source: "MToF"
---
# Duergar

Duergar, or gray dwarves, are grim and bitter denizens of the Underdark who were enslaved by mind flayers for millennia. Their suffering forged them into hardy, ruthless survivors with innate psionic magic that lets them grow in size and turn invisible.

## Traits
- **Ability Score Increase:** Constitution +2, Strength +1
- **Size:** Medium
- **Speed:** 25 ft. (not reduced by heavy armor)
- **Superior Darkvision:** 120 ft.
- **Dwarven Resilience:** Advantage on saving throws against poison; resistance to poison damage
- **Duergar Resilience:** Advantage on saving throws against illusions and against being charmed or paralyzed
- **Duergar Magic:** Cast [[Enlarge-Reduce|Enlarge/Reduce]] (enlarge only) at 3rd level and [[Invisibility]] at 5th level (once per long rest each, no components required)
- **Sunlight Sensitivity:** Disadvantage on attack rolls and Perception checks in direct sunlight

---
🔗 [Full Details on 5e.tools](https://5e.tools/races.html#duergar_mtof)
